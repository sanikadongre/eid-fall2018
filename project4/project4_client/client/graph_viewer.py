...
import datetime
from websocket import create_connection #for websockets
import matplotlib.pyplot as plt
import paho.mqtt.client as mqtt #for mqtt
import boto3
import ast
from aiocoap import * #for coap
import matplotlib
import json
...
class Ui_MainWindow(object):
    ...
    def time_protocols(self): #function to calculate the execution time for protocols
        ...
        print('The exchange time for RabbitMQ is: %s'% rabbit_execution_time)
        print('\nnumber messages: %d'% self.readings)
    ...
    def plotGraph(self):
        ...
        plt.plot(range(self.readings), self.min_temperature_data, 'c-', label='Min Temp')  #blue
        plt.plot(range(self.readings), self.present_temperature_data, 'm-', label='Last Temp')  #green
        ...
        plt.plot(range(self.readings), self.min_humidity_data, 'c-', label='Min Hum')  #blue
        plt.plot(range(self.readings), self.present_humidity_data, 'm-', label='Last Hum') #green
    ...
    def plotoutputprotocol(self): #funcion for plotting the protocols
        plt.plot(self.message_num, self.mqtt_time, 'g-', label='P1: MQTT')
        plt.plot(self.message_num, self.coap_time, 'y-', label='P2: CoAP')
        plt.plot(self.message_num, self.websockets_time, 'r-', label='P3: Websockets')
        plt.plot(self.message_num, self.rabbitmq_time, 'm-', label='P4: RabbitMQ')
        plt.legend(loc='best')
...
if __name__ == "__main__":
    ...
    client = mqtt.Client()
    client.connect('test.mosquitto.org',1883,60) #using mosquitto broker
    calcthread = [] #threads for parallel execution
    mqttthread = threading.Thread(target=func_mqtt) #mqtt thread
    ...
    rabbitmqthread.start() #rabbitmqthread
